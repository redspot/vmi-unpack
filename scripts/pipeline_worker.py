...
def fix_and_save_vads(_dump_files, _workdir, _outdir):
    for _file in _dump_files:
        ...
        if slot is not None:
            call_fix_binary(_file, _workdir, slot, basename, _outdir)
...
def main(sample_fd, domain, snapshot, outdir, dry_run):
    '''
    make a simple pipeline
        one python pusher for one sample file
            cmdline:
                sample file path
                domain name
                snapshot name
                output dir
        steps:
            kill domain if running
            get domain info
                qcow_fn
            start domain from snapshot
            short 5s poll loop for dom.IsActive
            get domain info
                agent_ip
                egress_nic
            create Agent
            wait for api ping
            upload sample
            start tcpdump (needs cap_net_admin+ep)
            start unpack (needs sudo)
            exec sample
            wait for unpack to exit, 8min
            kill tcpdump
            send unpack SIGINT
            wait for unpack to exit, 5min
            send SIGKILL if still running
            kill domain
            find_vad.py  # probably not needed with procdump
            stitch each dump
            produce:
                unpack log
                output json files
                procdump exes
                stitched exes
                    skip procdump/stitched exes if impscan finds no diff
                pcap
    '''
    rc = RC_SUCCESS
    ...
    try:
        ...
        logger.error(f"error: cannot find domain {domain}")
        return RC_NX_DOMAIN
    kill_domain(dom)
    xml = dumpxml(dom)
    qcow_fn = get_disk_fn(xml)
    rc = revert_snapshot(qcow_fn, snapshot)
    if rc:
        logger.error(f"error: cannot revert"
                     f" disk file {qcow_fn}"
                     f" to snapshot {snapshot}")
        if rc == -1:
            return RC_REVERT_TIMEOUT
        return RC_REVERT_FAILED
    time.sleep(1)
    with TemporaryDirectory(dir='/home/wmartin45/borg-out/tmp') as workdir,\
            ExitStack() as stack:
        stack.callback(chown_workdir, workdir)
        with LibvirtVM(domain) as vm:
            if not vm.is_running():
                rc, mesg = vm.get_error()
                logger.error(mesg)
                return rc
            guest_ip, egress_nic = get_network_info(dom, xml)
            if guest_ip is None:
                logger.error('agent guest_ip is None')
                return RC_NO_AGENT_IP
            agent = create_agent(guest_ip)
            logger.info("pinging agent ...")
            ping_agent(agent, timeout=30)
            time.sleep(1)
            upload_sample(agent, sample_fd)
            pcap_fn = outdir / f"{egress_nic}.pcap"
            with CapturePackets(pcap_fn, egress_nic):
                process_name = 'c1a73266.exe'
                unpack_log = Path(outdir) / 'unpack.log'
                logger.debug(f"opening fifo and log [{str(unpack_log)}]")
                with open(unpack_log, 'w') as log_fd,\
                        tempfifo(workdir) as fifo_fn:
                    with RunUnpack(
                            domain, process_name, workdir,
                            '-i', fifo_fn,
                            launcher_bin='sudo',
                            logfile=log_fd,
                            dryrun=dry_run,
                            exit_wait=(60)) as unpack:
                        logger.debug("sleeping a bit...")
                        time.sleep(3)
                        if not wait_for_fifo(fifo_fn, dry_run=dry_run):
                            logger.error(
                                    f"timeout waiting for fifo {fifo_fn}")
                            return RC_FIFO_TIMEOUT
                        time.sleep(1)
                        exec_start_ts = datetime.now()
                        with WatchSampleExec(agent):
                            sample_timeout_slice = 30
                            sample_timeout_tries = 30
                            failed = watch_unpack_and_vm(
                                    unpack, vm,
                                    timeout_slice=sample_timeout_slice,
                                    timeout_tries=sample_timeout_tries)
                            if failed:
                                rc += failed
                        # watch_exec is dead
        ...
        fix_and_save_vads(dumpfiles, workdir, outdir)
    # workdir is deleted
    return rc, exec_runtime


@click.command()
@click.option('-f', '--file', 'sample_fd', required=True,
              type=click.File('rb'))
@click.option('-d', '--domain', required=True)
@click.option('-s', '--snapshot', required=True)
@click.option('-o', '--outdir', required=True,
              type=click.Path(exists=True))
@click.option('--dry-run', is_flag=True)
def cli(sample_fd, domain, snapshot, outdir, dry_run):
    start_ts = datetime.now()
    logger.info(f"starting pipeline_worker:"
                f" sample={sample_fd.name}"
                f" domain={domain}"
                f" snapshot={snapshot}"
                f" outdir={outdir}")
    rc = RC_SUCCESS
    exec_rt = None
    try:
        rc, exec_rt = main(sample_fd, domain, snapshot, outdir, dry_run)
    except Exception:
